...
- **RE-SCOPE to within-source (feasibility grid, 2026-07-06; Pi Q5 branch taken).** The wall-clock feasibility grid (`wallclock-feasibility-grid-results.md`) shows **no usable common cross-source horizon**: SCID's discriminating band is ~**30–730 d** (target occupancy grows with W; empty/saturated both low), MIMIC's is ~**1 d only** (dense short admissions → ≥63% saturated by 3 d, ≥90% by 7 d; occupancy frozen at median 49 from 7 d on). The sole technically-common horizon (W=1 d) is **degenerate on both sides** (MIMIC 22% saturated, SCID 38% empty with median-1-event targets). Therefore the rung-0 hierarchy claim is made **within each source at its own discriminating band** (SCID ~30–730 d; MIMIC ~1–3 d), NOT at a common cross-source horizon. **Cross-source (MIMIC↔SCI-D) is a transportability diagnostic only** — never a common-horizon hierarchy claim.
