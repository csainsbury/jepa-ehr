...
def run_beta(m,p,seed,*,build_provenance_sha256,approved_envelope_sha256,phase_callback:Callable[[str],None]|None=None):
 ...
 e=stage1.TrainedCondition("L1_AVG",e0,None,None,{});stage1.freeze_encoder(e);stage1.freeze_encoder(l1);n("READOUT");y=probe.S[:,0].astype(np.uint8);tr0,c0=train_probe_trajectory(one.pooled(e,probe,t),y,m.readout_seed);tr1,c1=train_probe_trajectory(one.pooled(l1,probe,t),y,m.readout_seed)
 if c0!=c1 or tr0[250]["probe_state_bytes"]==tr0[2000]["probe_state_bytes"] or tr1[250]["probe_state_bytes"]==tr1[2000]["probe_state_bytes"] or full._state_dict_bytes(e0)!=before:raise PrototypeInvariantError("READOUT_INVALID")
 ycal=cal.S[:,0].astype(np.uint8);zc={"E0":one.pooled(e,cal,t),"L1":one.pooled(l1,cal,t)};calout={};scores={}
 ...
 rows=np.vstack((scores["L1","short"]-scores["E0","short"],scores["L1","long"]-scores["E0","long"]));rows=np.vstack((rows,rows[1]-rows[0]));reference,td=stage1.threshold_reference_report();constraint_rows,_=stage1.trained_collapse_diagnostics(l1,cal,t,reference);cp=all(x["both_metrics_pass"] for x in constraint_rows)
