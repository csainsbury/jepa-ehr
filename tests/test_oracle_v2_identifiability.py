...
class FrozenParams(unittest.TestCase):
    def test_impl_identity_and_params(self) -> None:
        ...
        self.assertEqual(idf.D_VECTOR, ("S3_tau", "S3_loggap", "S4_abs", "S7_abs"))
...
class PureNumerics(unittest.TestCase):
    def test_standardized_rank(self) -> None:
        Sig = np.eye(4)
        J_good = np.zeros((4, 3)); J_good[np.arange(3), np.arange(3)] = 10.0    # full column rank
    ...
    def test_nearest_grid_recovery(self) -> None:
        ...
        vecs = [np.full(4, v) for v in (0.1, 0.35, 0.55)]
        rec = idf.nearest_grid_recovery(np.full(4, 0.34), thetas, vecs)
    ...
    def test_collision_search(self) -> None:
        ...
        same = [np.zeros(4), np.zeros(4)]                                        # identical vectors, far apart
        self.assertTrue(idf.collision_search(far, same, accept_tol=0.01))
        distinct = [np.zeros(4), np.full(4, 1.0)]
    ...
    def test_frozen_nuisance_and_accept_tol(self) -> None:
        ...
        import numpy as np
        self.assertTrue(np.allclose(idf.ACCEPT_TOL, [0.05, np.log(1.10), 0.03, 0.03]))
...
class Pipeline(unittest.TestCase):
    def test_f_theta_jacobian_and_rank(self) -> None:
        ...
        self.assertEqual(z.shape, (4,))                           # 4-scalar D vector (S6_tv dropped)
        ...
        self.assertEqual(J.shape, (4, 3))                         # 4 stats x 3 components
