...
def manifest(path="README.md"):
 return {"schema_version":"cog-project-manifest-v1","project":{"slug":"jepa","repository":"csainsbury/jepa-ehr"},"transport":{"repository":"csainsbury/cog-continuity-inbox"},"allowed_refs":["refs/heads/cog/jepa-continuity-pilot","refs/heads/docs/rung-minus1-readiness"],"documents":[{"path":path,"label":"Overview","role":"resume","classification":"safe_distilled"}],"limits":{"max_file_bytes":262144,"max_total_bytes":524288}}

class EmitterTests(unittest.TestCase):
 def setUp(self):
  self.t=tempfile.TemporaryDirectory();self.root=Path(self.t.name);run(self.root,"init","-b","docs/rung-minus1-readiness");run(self.root,"config","user.email","test@example.invalid");run(self.root,"config","user.name","Test")
 ...
 def build(self,ref="refs/heads/docs/rung-minus1-readiness"):
