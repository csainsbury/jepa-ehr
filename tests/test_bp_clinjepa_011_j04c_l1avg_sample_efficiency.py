...
def execution_provenance():
 digest="0"*64;runtime={"python_version":"3","numpy_version":"2","torch_version":"2","platform_machine":"x","platform_system":"Linux","blas_fingerprint":"sha256:"+"1"*64}
 return assay.provenance_from_dict({"schema":"BP011-J04C-L1AVG-SAMPLE-EFFICIENCY-N256-BUILD-PROVENANCE-V1","target_commit":assay.TARGET_COMMIT,"implementation_commit":"a"*40,"clean_tree":True,"source_digests":assay.SOURCE_DIGESTS,"implementation_digests":{p:digest for p in assay.IMPLEMENTATION_PATHS},**runtime})
def test_execution_environment_rejects_head_mismatch(monkeypatch):
 provenance=execution_provenance();monkeypatch.setattr(assay,"_command",lambda *args:"b"*40 if args[1]=="rev-parse" else "");monkeypatch.setattr(assay.subprocess,"check_call",lambda *args,**kwargs:0);monkeypatch.setattr(assay,"_file_sha256",lambda path:({**provenance.source_digests,**provenance.implementation_digests})[path]);monkeypatch.setattr(assay,"runtime_fingerprint",lambda:{k:getattr(provenance,k) for k in ("python_version","numpy_version","torch_version","platform_machine","platform_system","blas_fingerprint")})
 with pytest.raises(assay.PrototypeInvariantError,match="PROVENANCE_CONTENT"):assay.verify_execution_environment(provenance)
def test_execution_environment_rejects_source_or_implementation_byte_mismatch(monkeypatch):
 provenance=execution_provenance();digests={**provenance.source_digests,**provenance.implementation_digests};bad_path=next(iter(digests));monkeypatch.setattr(assay,"_command",lambda *args:provenance.implementation_commit if args[1]=="rev-parse" else "");monkeypatch.setattr(assay.subprocess,"check_call",lambda *args,**kwargs:0);monkeypatch.setattr(assay,"runtime_fingerprint",lambda:{k:getattr(provenance,k) for k in ("python_version","numpy_version","torch_version","platform_machine","platform_system","blas_fingerprint")});monkeypatch.setattr(assay,"_file_sha256",lambda path:digests[path]);assert assay.verify_execution_environment(provenance).source_pins is True
 monkeypatch.setattr(assay,"_file_sha256",lambda path:"f"*64 if path==bad_path else digests[path])
 with pytest.raises(assay.PrototypeInvariantError,match="PROVENANCE_CONTENT"):assay.verify_execution_environment(provenance)
