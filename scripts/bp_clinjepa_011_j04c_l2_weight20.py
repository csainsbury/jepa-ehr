#!/usr/bin/env python3
"""Restore L1-equivalent per-identity hinge strength for 16-identity L2 by scaling weight 5→20."""
...
G=1102;MODELS=(2101,2102,2103);READOUT=2102;RECIPE='L2_SEP';WEIGHT=20.;FLOOR=.01;UNTRAINED=(0.7753685818549992,0.7805427899230244,0.75101867607262)
...
def main():
 ...
 means=[sum(v)/3 for v in values];deltas=[a-b for a,b in zip(means,UNTRAINED)];ranges=[max(v)-min(v) for v in values];checks={'all_sixteen_identity_constraints_all_three_seeds':all(v['constraint_transfer_all_pass'] for v in results.values()),'all_factor_ba_at_least_0_80':min(x for row in values for x in row)>=.8,'each_factor_seed_range_at_most_0_05':max(ranges)<=.05,'trained_minus_untrained_mean_at_least_0_05_each_factor':min(deltas)>=.05}
 print(json.dumps({'schema':'BP011-J04C-L2-WEIGHT20-V1','contract':{'axis':'scale L2 hinge weight 5 to 20 to preserve L1 per-identity gradient strength when identity count rises 4 to 16','pass':'all 16 constraints pass in 3/3; all BA >=0.80; each factor range <=0.05; trained-minus-untrained mean >=0.05 per factor','no_tuning_after_open':True},'generator_seed':G,'model_seeds':MODELS,'common_readout_seed':READOUT,'recipe':RECIPE,'directional_variance_weight':WEIGHT,'directional_variance_floor':FLOOR,'untrained_factor_ba_means_exact':UNTRAINED,'trained_factor_ba_by_factor':values,'trained_factor_ba_means':means,'trained_minus_untrained_mean_ba':deltas,'factor_seed_ranges':ranges,'checks':checks,'contract_pass':all(checks.values()),'results':results,'threshold_digest':digest,'claim_ceiling':'FIXED_GENERATOR_PUBLIC_SYNTHETIC_L2_RECIPE_BETA'},sort_keys=True,separators=(',',':'),allow_nan=False))
