...
MODELS=(2101,2102,2103);HELDOUT=((7200,(.5,.5,.5),(.10,.22,.18)),(7201,(.5,.5,.5),(.22,.10,.18)),(7202,(.5,.5,.5),(.18,.22,.10)));Q_GLOBAL=.8887;Q_LOCAL=.8017
...
def seed_path_audit():
 ...
 other|={(seed,epoch,6101) for seed in MODELS for epoch in range(32)};other|={(READOUT,factor,epoch,6201) for factor in range(3) for epoch in range(32)};other|={(seed,split) for seed in (1101,1102,1103,4100,4101,4102,5100,5101,5102) for split in (PROBE_FIT,CAL_OOD)}
 if row&other or len(row)!=6:raise RuntimeError('row seed-sequence path collision')
 return {'row_paths':[list(x) for x in sorted(row)],'known_other_path_count':len(other),'exact_path_intersection':[]}
...
def main():
 ...
 for hseed,sp,xp in HELDOUT:
  ...
  for seed in MODELS:
   ...
   rows,position=trained_collapse_diagnostics(conditions[str(seed)]['trained'],cal_split,transform,reference);cells.append({'heldout_seed':hseed,'s_probabilities':sp,'signal_flip_probabilities':xp,'model_seed':seed,'probe_fit_digest':pd,'cal_ood_digest':cd,'arms':arms,'trained_q_unclipped':trained_q,'paired_gains':[arms['trained'][f]['balanced_accuracy']-arms['untrained'][f]['balanced_accuracy'] for f in range(3)],'constraints_all_pass':all(r['both_metrics_pass'] for r in rows),'constraint_rows':rows,'position_specificity_descriptive':position})
