#!/usr/bin/env python3
"""Test the first nonzero full-L2 increment (beta=0.1) from the last-green L1 recipe."""
...
def main():
 ...
 for seed in MODELS:
  c=train_l1_plus_l2_beta(train,transform,seed,l2_beta=BETA,total_steps=2000);freeze_encoder(c);probes,head,_=fit_condition_readouts(c,probe,transform,READOUT);ev,_=evaluate_readouts(c,probes,head,cal,transform);ba=[f['balanced_accuracy'] for f in ev['factors']];rows,position=trained_collapse_diagnostics(c,cal,transform,reference)
 ...
 print(json.dumps({'schema':'BP011-J04C-L1-ANCHORED-L2-BETA01-V1','contract':{'axis':'add the first nonzero full four-layer L2 objective at beta 0.1 to the last-green L1 objective','genuine_l2':'all 16 separated targets contribute nonzero cosine and directional-constraint gradients','pass':'all 16 L2 constraints pass in 3/3; all BA >=0.80; factor ranges <=0.05; mean trained-minus-untrained >=0.05 per factor','no_tuning_after_open':True},'generator_seed':G,'model_seeds':MODELS,'common_readout_seed':READOUT,'recipe':'L1_AVG+0.1*L2_SEP','l2_beta':BETA,'l1_directional_weight':5.0,'l2_directional_weight':20.0,'directional_floor':0.01,'trained_factor_ba_by_factor':values,'trained_factor_ba_means':means,'trained_minus_untrained_mean_ba':deltas,'factor_seed_ranges':ranges,'checks':checks,'contract_pass':all(checks.values()),'results':results,'threshold_digest':digest,'claim_ceiling':'FIXED_GENERATOR_PUBLIC_SYNTHETIC_L1_ANCHORED_NONZERO_L2_BETA'},sort_keys=True,separators=(',',':'),allow_nan=False))
