#!/usr/bin/env python3
"""Raise the direct student-variance floor from 0.05 to 0.10 after the lower floor missed OOD variance."""
...
GENERATOR_SEED=1102; MODEL_SEEDS=(2101,2102,2103); COMMON_READOUT_SEED=2102; WEIGHT=0.01; FLOOR=0.10
...
def main():
 ...
 for seed in MODEL_SEEDS:
  condition=train_l0_decoupled_seeds(train,transform,seed,seed,schedule_seed=seed,identity_predictor=True,total_steps=2000,student_variance_weight=WEIGHT,student_variance_floor=FLOOR); freeze_encoder(condition); probes,head,_=fit_condition_readouts(condition,probe,transform,COMMON_READOUT_SEED); evaluation,_=evaluate_readouts(condition,probes,head,cal,transform); ba=[f['balanced_accuracy'] for f in evaluation['factors']]; rows,_=trained_collapse_diagnostics(condition,cal,transform,reference); composition.append(ba[0]); results[str(seed)]={'factor_ba':ba,'collapse_all_pass':all(r['both_metrics_pass'] for r in rows),'collapse_rows':rows,'training_loss_summary':condition.training['losses']}
 ...
 print(json.dumps({'schema':'BP011-J04C-L0-STUDENT-VARIANCE-FLOOR010-V1','contract':{'axis':'raise direct student pooled-variance floor from 0.05 to 0.10 at fixed weight 0.01','pass':'collapse 3/3, composition BA >=0.80 in 3/3, range <=0.05','not_tested':'direct superiority or target-family changes'},'generator_seed':GENERATOR_SEED,'model_seeds':MODEL_SEEDS,'common_readout_seed':COMMON_READOUT_SEED,'student_variance_weight':WEIGHT,'student_variance_floor':FLOOR,'composition_ba':composition,'composition_ba_range':max(composition)-min(composition),'checks':checks,'contract_pass':all(checks.values()),'results':results,'threshold_digest':digest,'claim_ceiling':'PUBLIC_SYNTHETIC_STUDENT_VARIANCE_FLOOR_INTERVENTION'},sort_keys=True,separators=(',',':'),allow_nan=False))
