#!/usr/bin/env python3
"""Single frozen moderate-prevalence beta for the generator-1102 L1 recipe."""
...
MODELS=(2101,2102,2103);HELDOUT=((9200,(.425,.525,.575),(.15,.15,.15)),(9201,(.575,.475,.425),(.15,.15,.15)),(9202,(.525,.575,.475),(.15,.15,.15)));Q_GLOBAL=.8887;Q_LOCAL=.8017
...
def seed_path_audit():
 ...
 categories={'prior_row_paths':{(seed,split) for seed in (1101,1102,1103,4100,4101,4102,5100,5101,5102,7200,7201,7202,8200,8201,8202) for split in (PROBE_FIT,CAL_OOD)},'training_data_paths':{(1102,TRAIN)},'nuisance_paths':{(1102,TRAIN,7101)},'training_schedule_paths':{(seed,epoch,6101) for seed in MODELS for epoch in range(32)},'readout_schedule_paths':{(READOUT,factor,epoch,6201) for factor in range(3) for epoch in range(32)}}
...
def main():
 ...
 expected_heldout=((9200,(.425,.525,.575),(.15,.15,.15)),(9201,(.575,.475,.425),(.15,.15,.15)),(9202,(.525,.575,.475),(.15,.15,.15)))
