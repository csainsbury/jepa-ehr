#!/usr/bin/env python3
"""Result-contingent moderate-nonuniform-corruption L1_AVG beta."""
from __future__ import annotations
import hashlib,json,math,sys,numpy as np,torch
...
MODELS=(2101,2102,2103);HELDOUT=((11200,(.425,.525,.575),(.125,.185,.165)),(11201,(.575,.475,.425),(.185,.125,.165)),(11202,(.525,.575,.475),(.165,.185,.125)));Q_GLOBAL=.8887;Q_LOCAL=.8017
...
PRIOR_CORRUPTION_MILLIUNITS={11200:(100,220,180),11201:(220,100,180),11202:(180,220,100)}
EXPECTED_CORRUPTION_MILLIUNITS={11200:(125,185,165),11201:(185,125,165),11202:(165,185,125)}
EXPECTED_Q_DENOMINATOR_MILLIUNITS={11200:(375,315,335),11201:(315,375,335),11202:(335,315,375)}
EXPECTED_Q_DENOMINATORS={seed:tuple(x/1000 for x in values) for seed,values in EXPECTED_Q_DENOMINATOR_MILLIUNITS.items()}
...
def seed_path_audit():
 ...
 categories={'prior_row_paths':{(seed,split) for seed in (1101,1102,1103,4100,4101,4102,5100,5101,5102,7200,7201,7202,8200,8201,8202,9200,9201,9202,10200,10201,10202) for split in (PROBE_FIT,CAL_OOD)},'training_data_paths':{(1102,TRAIN)},'nuisance_paths':{(1102,TRAIN,7101)},'training_schedule_paths':{(seed,epoch,6101) for seed in MODELS for epoch in range(32)},'readout_schedule_paths':{(READOUT,factor,epoch,6201) for factor in range(3) for epoch in range(32)}}
...
def main():
 ...
 expected_heldout=((11200,(.425,.525,.575),(.125,.185,.165)),(11201,(.575,.475,.425),(.185,.125,.165)),(11202,(.525,.575,.475),(.165,.185,.125)))
 if runtime!=EXPECTED_RUNTIME or MODELS!=(2101,2102,2103) or READOUT!=2102 or HELDOUT!=expected_heldout or (PROBE_FIT,CAL_OOD)!=(6,3):raise RuntimeError('runtime or exact contracted joint contract mismatch')
 actual_corruption_milliunits={seed:tuple(int(round(p*1000)) for p in xp) for seed,_,xp in HELDOUT}
 midpoint_corruption_mapping_pass=actual_corruption_milliunits==EXPECTED_CORRUPTION_MILLIUNITS and all(EXPECTED_CORRUPTION_MILLIUNITS[seed]==tuple(150+(old-150)//2 for old in PRIOR_CORRUPTION_MILLIUNITS[seed]) for seed in EXPECTED_CORRUPTION_MILLIUNITS) and all(math.isclose(p,milli/1000,rel_tol=0,abs_tol=1e-15) for seed,_,xp in HELDOUT for p,milli in zip(xp,EXPECTED_CORRUPTION_MILLIUNITS[seed]))
 if not midpoint_corruption_mapping_pass:raise RuntimeError('contracted corruption midpoint mapping mismatch')
 ...
 cells=[];q_denominator_mapping_passes={}
 for hseed,sp,xp in HELDOUT:
  ...
  q_denominators=EXPECTED_Q_DENOMINATORS[hseed]
  q_denominator_mapping_passes[str(hseed)]=all(math.isclose(.5-xp[f],q_denominators[f],rel_tol=0,abs_tol=1e-15) for f in range(3))
  if len(probe_split.S)!=2048 or len(cal_split.S)!=2048 or not q_denominator_mapping_passes[str(hseed)]:raise RuntimeError('split row count or factor-specific Q denominator mapping mismatch')
  ...
  for seed in MODELS:
   ...
   trained_q=[(arms['trained'][f]['balanced_accuracy']-.5)/q_denominators[f] for f in range(3)]
