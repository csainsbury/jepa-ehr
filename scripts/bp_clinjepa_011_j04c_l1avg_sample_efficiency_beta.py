...
def main(argv=None):
 ...
 try:
  ...
  seed=assay.validate_seed(manifest,mr,envelope,iv,ir);result,raw=assay.run_beta(manifest,provenance,seed,build_provenance_sha256=args.build_provenance_sha256,approved_envelope_sha256=args.approved_seed_envelope_sha256,phase_callback=lambda phase:state.__setitem__("phase",phase));write_arrays(args.retained_array_dir,raw);sys.stdout.buffer.write(assay.canonical(result)+b"\n");return 0
